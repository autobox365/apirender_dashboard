...
const PD3DExteriorRenders360Degree = ({ service, orders, history }) => {
  ...
  const handleSubmit = (event) => {
    ...
    files.map((file) => { postFormData.append("PD_3D_EXTERIOR_RENDERS_360_DEGREE_PHOTO_MONTAGE", file)});

    files = incorporateEnvironmentDropzone.current.myDropzone.files;
    files.map((file) => { postFormData.append("PD_3D_EXTERIOR_RENDERS_360_DEGREE_INCORPORATE_ENVIRONMENT", file)});
